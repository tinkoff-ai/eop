...
def _any2cuda_non_blocking(value: Any, device: torch.device):
    # based on catalyst.utils.torch.any2device
    # but with cuda non_blocking trick
    if isinstance(value, dict):
        return {k: _any2cuda_non_blocking(v, device) for k, v in value.items()}
    elif isinstance(value, (tuple, list)):
        return [_any2cuda_non_blocking(v, device) for v in value]
    elif torch.is_tensor(value):
        return value.to(device, non_blocking=True)
    elif isinstance(value, (np.ndarray, np.void)) and value.dtype.fields is not None:
        return {k: _any2cuda_non_blocking(value[k], device) for k in value.dtype.fields.keys()}
    elif isinstance(value, np.ndarray):
        return torch.tensor(value).to(device, non_blocking=True)
...
def _prefetch_map(
    func: Callable, iterable: Iterable, device: torch.device, num_prefetches: int = 1, timeout: int = 2,
) -> Iterable:
    result_queue = queue.Queue(num_prefetches)
    error_queue = queue.Queue(1)
    done_event = threading.Event()
    map_thread = threading.Thread(
        target=_map_loop, args=(func, iterable, device, result_queue, error_queue, done_event),
    )
...
def _prefetch_loader(loader: DataLoader, num_prefetches: int, device: torch.device) -> Iterable:
    if torch.cuda.is_available():
        return _prefetch_map(_any2cuda_non_blocking, loader, num_prefetches=num_prefetches, device=device)
    else:
        return iter(loader)
...
class BatchPrefetchLoaderWrapper(ILoaderWrapper):
    ...
    def __init__(self, loader: DataLoader, device: torch.device, num_prefetches: int = None):
        """Loader wrapper. Prefetches specified number of batches on the GPU.
    ...
    def __iter__(self):
        ...
        Returns:
            iterator object
        """
        return _prefetch_loader(self.origin, self.num_prefetches, self.device)
